...
def _part_train_skips(part: Part) -> list:
    return [part.primitives, part.media_preview]


def _train_skips(parts: Union[Part, Tuple[Part, ...]], **_) -> list:
    ...
    ``TrainStack.train_track`` also accepts a tuple accumulation window; preserve
    that pytree shape while selecting the unread payloads from every ``Part``.
    """
    if isinstance(parts, tuple):
        return [_part_train_skips(part) for part in parts]
    return _part_train_skips(parts)
