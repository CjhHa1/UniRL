...
- ``patch_sigmas_passthrough`` — forwards ``sampling_params.sigmas`` into
  ...
  DELETE-WHEN: upstream pipeline forwards ``sigmas`` itself. Still needed at
  v0.27.0rc1: the inner ``__call__`` takes ``sigmas`` but ``forward`` never
  passes it, and only the separate step-execution path rejects sigmas.
