...
| `patch_qwen3_omni_thinker_lora` | Compatibility bundle around vllm-omni #3915 plus UniRL's audio truncation and audio-video M-RoPE fixes; the LoRA class patch self-skips on the current v0.22 pin | the audio fixes are upstream and the remaining class patch is deleted |
