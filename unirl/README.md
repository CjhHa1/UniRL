...
- **Entrypoints** (`train_diffusion.py`, `train_ar.py`, `train_pe.py`,
  `train_unified_model.py`, plus the domain-dispatched `eval.py` for one-shot
  ckpt scoring) — each composes and validates the Hydra recipe, then hands off
  to its trainer.
...
| `eval.py` | Unified one-shot eval entrypoint (`--domain ar\|diffusion`; `run_eval`, no train loop) |
