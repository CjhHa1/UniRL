...
class SD3DiffusionStage(DiffusionStage[SD3Conditions]):
    ...
    def __init__(
        ...
        *,
        ...
        # all-gathers per replay down to 1. Safe ONLY for stateless,
        # step_index-independent SDE strategies (Flow/Dance/CPS); the stateful
        # DPM2 ODE solver is not an SDEStrategy and never reaches the SDE-logp
        # path anyway. Default OFF preserves the exact serial path for every
        # other recipe. Under ``old_logp_source='replay'`` both the π_old anchor
        # (prepare_segment) and the train forward go through this same
        # ``replay``, so when batching is on both use the identical [S*B]
        # geometry -> the on-policy ratio stays exactly 1. (With
        # ``old_logp_source='rollout'`` the anchor is the engine's emission, not
        # replayed, so that exact-ratio guarantee does not come from here.)
