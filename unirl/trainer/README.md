...
| `AsyncARTrainer` | FIFO AR generation batch → one `TrainStack` | Separate train/rollout slabs with resident generation and optimizer-update versioning. `async_control_mode=unified` launches from the train loop; `dual` uses an independent bounded producer while the main thread scores and trains. Both quiesce before weight sync, eval, or checkpoint. |
