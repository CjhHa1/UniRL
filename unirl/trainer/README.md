...
| `AsyncARTrainer` | FIFO AR generation batch → one `TrainStack` | Separate train/rollout slabs with resident generation, batch-denominated `max_staleness` admission over an optimizer-update clock, and quiescence before weight sync, eval, or checkpoint. |
