...
| `AsyncARTrainer` | FIFO AR generation batch → one `TrainStack` | Separate train/rollout slabs with resident generation and optimizer-update versioning. `async_control_mode=unified` admits active work only; `dual` prequeues bounded work so the manager's existing dispatch thread keeps generation active while the main thread scores and trains. Both quiesce before weight sync, eval, or checkpoint. |
