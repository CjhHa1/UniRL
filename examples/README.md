...
Each domain maps to a training entrypoint and, where supported, a selector on the
unified eval entrypoint. The **default recipe** below is the training entrypoint's
built-in `config_name`; pass `--config-name` explicitly when evaluating.

| Domain | Entrypoint | Default recipe (start here) | Models |
|---|---|---|---|
| [`diffusion/`](diffusion/) | `python -m unirl.train_diffusion` / `python -m unirl.eval --domain diffusion` | `diffusion/sd3/sd3_trainside` | `sd3`, `qwen_image`, `flux2_klein`, `wan21`, `wan22`, `hunyuan_video`, `hunyuan_video15` |
| [`ar/`](ar/) | `python -m unirl.train_ar` / `python -m unirl.eval --domain ar` | `ar/qwen_vl_grpo_geo3k_mc_4x8`, `ar/qwen3_drpo_4b_base_dapo_sglang` | `qwen_vl` (vision-language), `qwen3` (text-only) |
...
The eval entrypoint selects the trainer with `--domain` and calls
`trainer.run_eval()` once (no train loop). Pass `+load_dir=<checkpoint_dir>` for
a trained adapter, or set the recipe's base-model env (`PRETRAINED_MODEL` /
`QWEN_VL_PATH`) to evaluate a full-weight ckpt. See
[FlowDPPO/README.md](../FlowDPPO/README.md#evaluate-a-checkpoint).
