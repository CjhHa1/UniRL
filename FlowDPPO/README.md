...
Standalone eval (no train loop) via `python -m unirl.eval --domain diffusion`.
In-domain vs out-of-domain is just a different `eval_data_path` (and optionally
a different `reward` scorer).

```bash
# Full-weight HF / exported ckpt (base via PRETRAINED_MODEL)
PRETRAINED_MODEL=<hf-or-local-ckpt> \
python -m unirl.eval --domain diffusion --config-name=diffusion/sd3/sd3_flowdppo \
